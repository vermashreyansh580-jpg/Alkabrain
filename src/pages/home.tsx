...
import { useChatStore, type ChatMode, type ChatResponse } from "@/lib/store";
...
export function Home() {
  ...
  const generateMedia = async (
    ...
    try {
      const url = `${HF_BACKEND}/api/${kind}/generate`;
  ...
  const handleSend = async (
    ...
    try {
      ...
      const r = await fetch(`${HF_BACKEND}/api/chat`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const res: ChatResponse = await r.json();
    ...
    <ResizablePanelGroup direction="horizontal" className="h-full items-stretch flex-1 min-h-0">
      <ResizablePanel defaultSize={60} minSize={30} className="flex flex-col min-h-0 bg-background relative">
        ...
        <div className="absolute bottom-0 inset-x-0 p-4 bg-gradient-to-t from-background via-background to-transparent pt-10">
          <Composer
            onSend={handleSend}
            disabled={isSending || authLoading || !!mediaPending}
            mode={mode}
